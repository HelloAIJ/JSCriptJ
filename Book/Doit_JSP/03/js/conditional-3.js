...
let score = prompt("프로그래밍 점수를 숫자로만 기입해주세요.");
// console.log(score);
// console.log(parseInt(score)); // NaN
// 사용자가 숫자를 입력할 경우, 스트링이지만 parseInt()를 거치면 숫자로 바꿔줌. 그냥 문자열을 입력하면 NaN을 뱉어줌.

// if (isNaN(score)) {
//   alert(`다시 입력해주세요.`);
//   score = prompt(`점수를 재입력해주세요.(숫자만사용)`);
//   console.log(score);
//   console.log(parseInt(score));
// }
// console.log(typeof (parseInt(score))); // number
// typeof로 확인하는 값을 맹신하지말자, 문자열입력한것을 parseInt()를 거치면 무조건 number로 인식

/**
 * parseInt()를 거친 값이 숫자가 아니라면(문자열) NaN이나오지만,
...
if (score != null) {
  ...
  else if (isNaN(Int_score)) alert("문자 입력 금지");
} else alert("답변을 해주세요");
// 사용자가 취소를 누른 경우
